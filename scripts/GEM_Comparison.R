...
# LPS Gapdh Knockout
Gapdh_KO_LPS = read.csv(paste0(path, 'FVA_Gapdh_KO_LPS.csv'))
# COBRApy IL4 WT
IL4Cpy_WT = read.csv(paste0(path, 'FVA_IL4_WT.csv'))
# COBRApy IL4 KO
IL4Cpy_KO = read.csv(paste0(path, 'FVA_IL4_KO.csv'))
...
FluxSumarize = FluxSumarize[!grepl("Vitamin", FluxSumarize$Subsystem),] #for Poster plot
FluxSumarize = FluxSumarize[!grepl("Fatty", FluxSumarize$Subsystem),] #for Poster plot
FluxSumarize = FluxSumarize[!grepl("Glycos", FluxSumarize$Subsystem),] #for Poster plot
...
boxPlot = ggplot(FluxSumarize, aes(x=Flux, y=Subsystem, fill=Model)) +
  ...
  scale_fill_manual(values=c("WT" = WT_col, # explicitly map color to the factor
                             "Cyp27a1 KO" = col),
                    labels = c('WT', 'Cyp27a1 KO')) + # explicitly change legend text
  theme_minimal() +
...
SpecFluxesFun = function(WT_df, KO_df, FVA_WT,FVA_KO_df,
      ...
      if (WhichSubsystem == "Tricarboxylic acid cycle and glyoxylate/dicarboxylate metabolism") {
        
        df_insert = LPSGapFilled %>% dplyr::filter(ReactionName == "PDHm")
        df =  df %>% 
          rows_insert(df_insert)
        
      }
        
      df2 = LPS_Knockout_List[[WhichGene]] %>%
        dplyr::filter(Subsystem %in% WhichSubsystem) %>%
      ...
      if (WhichGene != "Gapdh") {
        df = df %>%
          dplyr::filter(Flux != 0) %>%
          rows_insert(LPSGapFilled %>% dplyr::filter(ReactionName %in% c('ICDHxm', 'AKGDm')))
        
        df2 = df2 %>%
          dplyr::filter(Flux != 0) %>%
          rows_insert(LPS_Knockout_List[["Cyp27a1"]] %>% dplyr::filter(ReactionName %in% c('ICDHxm', 'AKGDm')))
      ...
      if (WhichSubsystem == "Tricarboxylic acid cycle and glyoxylate/dicarboxylate metabolism") {
        ...
        df_insert2 = LPS_Knockout_List[[WhichGene]] %>% dplyr::filter(ReactionName == "PDHm")
        df_insert3 = LPS_Knockout_List[[WhichGene]] %>% dplyr::filter(ReactionName == "SUCOAS1m")
        df2 =  df2 %>%
          rows_insert(df_insert2)
      ...
      if (WhichSubsystem == "Tricarboxylic acid cycle and glyoxylate/dicarboxylate metabolism") {
        ...
        df2[nrow(df2) + 1, ] = c(0, -1000, 1000, "MAR04147","SUCOAS1m", 
                                 "Tricarboxylic acid cycle and glyoxylate/dicarboxylate metabolism",
                                 "CoA[m] + GTP[m] + succinate[m] <=> GDP[m] + Pi[m] + succinyl-CoA[m]",
                                 0)
      ...
      combined$Model = rep(c("WT", WhichGene), times = c(dim(df)[1], dim(df2)[1]))

      if (WhichSubsystem == "Tricarboxylic acid cycle and glyoxylate/dicarboxylate metabolism") {

        combined = combined %>%
          dplyr::filter(!ReactionName %in% c('r0509', 'SUCOASm', "MDH"))


      }
      
      combined$Flux = as.numeric(combined$Flux)
      combined$FVAMin = as.numeric(combined$FVAMin)
      combined$FVAMax = as.numeric(combined$FVAMax)
      combined$FVAHalfPoint = as.numeric(combined$FVAHalfPoint)
      ...
      cols = vctrs::vec_c(!!"Cyp27a1" := "#084594",
                          !!"WT" := "#56B4E9")

      fba = ggplot(BileAcid[["CombinedDataset"]], aes(x = #ifelse(WhichSubsystem == "Bile acid biosynthesis",
                                   ReactionID,
                                   #ReactionName, 
                                   y = Flux,
                           fill=Model)) +
        ...
        scale_fill_manual(values=cols,
                          labels = c(paste0("Cyp27a1", '_KO'), 'WT')) +
        coord_flip() +
      ...
      fva = ggplot(BileAcid[["CombinedDataset"]], aes(x = ReactionID, y = FVAHalfPoint,
...
#TCAMet = 
  SpecFluxesFun(LPSGapFilled, LPS_Knockout_List[["Cyp27a1"]], LPS, 
                       Cyp27a1_KO_LPS,
...
RepeatHILIC = read_excel(paste0(path, "/Repeat_Cyp.xlsx"),
                         sheet = 1)
RepeatRP = read_excel(paste0(path, "/Repeat_Cyp.xlsx"),
                         sheet = 2)

GAPDH = read.csv(paste0(path, "/GAPDH_experiment.csv"))

Third_Cyp_Experiment = read_excel(paste0(path, "/Third_CYP_experiment_raw_edited.xls"),
                                  sheet = 1)
...
PlotCombinedMets("IMP", "Hilic")


test = IL4_Knockout_List[["Cyp27a1"]] %>% dplyr::filter(Subsystem %in% "Purine metabolism")
...
#                                               "IL-4-WT", "IL-4-KO"))
# 
# rownames(annot) <- names(HilicMat)
# 
# MetsToKeep = c("Glucose6phosphate", "Fructose6phosphate", "Fructose-1,6-biphosphate",
#                "ATP", "Pyruvate", "Malate", "AcetylCoA", "Succinate", "NADH", "NAD")
# 
# pheatmap(HilicMat[MetsToKeep, ],
#              color = rev(colorRampPalette(c("#B2182B", "#FDDBC7", "#2166AC"))(n = 256)),
#              border_color = "gray40",
#              scale = "row",
#              gaps_col = c(8,16),
...
#Fix qPCR plots - Arg 1 and iNOS

setwd('/Users/rokosango/PhD/MetabModelling/MATLAB_scripts_workspaces/csv_txt_xlsx_files')
ArgDf = read_excel("qPCRs.xlsx", sheet = 1)
iNOSDf = read_excel("qPCRs.xlsx", sheet = 2)

colnames(ArgDf)[1:3] = c("Group", "FoldExpression", "Std")
colnames(iNOSDf)[1:3] = c("Group", "FoldExpression", "Std")

ggplot(iNOSDf[!grepl("48", iNOSDf$Group),], aes(x=Group, y=FoldExpression, fill=Group)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(aes(ymin=FoldExpression-Std, ymax=FoldExpression+Std), width=.2,
                position=position_dodge(.9)) + my.theme + labs(x = NULL, y = NULL,
                                                               title = "iNOS normalized to ACTB") + 
  scale_fill_manual(values=c("LPS" = "#56B4E9", # explicitly map color to the factor
                             "IL4" = "#E69F00",
                             "ctrl" = "#984EA3")) + 
...
# Different TPM barplots:

DiffTpms = read.csv("DiffTPMs.csv")



ggplot(data=DiffTpms, aes(x=Model, y=NumGenes, fill=Model)) +
  geom_bar(stat="identity", color="black", position=position_dodge()) +
...
MouseGEM_genes = read_tsv('/Users/rokosango/PhD/MetabModelling/MATLAB_scripts_workspaces/MouseGEM_genes.tsv')
ENSMUS_genes = read.csv('/Users/rokosango/PhD/MetabModelling/MATLAB_scripts_workspaces/gProfiler_mmusculus_21-02-2024_13-27-55.csv')


combined_df = merge(MouseGEM_genes, ENSMUS_genes, by = "name")

dim(combined_df) #2943
dim(combined_df[!duplicated(combined_df$name), ]) #2728

combined_df = combined_df[!duplicated(combined_df$name), ]

write_tsv(combined_df, "genes.tsv")
...
df = read_excel(paste0(path, "/Third_CYP_experiment_raw_edited.xls"),
                                  sheet = 1)
